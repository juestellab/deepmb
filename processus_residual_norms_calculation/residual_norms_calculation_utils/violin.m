...
%            ksdensity()
% support:   support parameter that is passed to ksdensity()
% OUTPUT
%
% h:     figure handle
% L:     Legend handle
...
if find(strcmp(varargin,'bw')) && ~isempty(varargin{find(strcmp(varargin,'bw'))+1})
...
if find(strcmp(varargin,'support'))
    support = varargin{find(strcmp(varargin,'support'))+1};
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    if ~isempty(b)
        [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'Support', support, 'Kernel', 'epanechnikov');
    else
        [f, u, bb]=ksdensity(Y{i},'Support', support, 'Kernel', 'normal', 'NumPoints',100 );
    end
...
for i=i:size(Y,2)
    ...
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],'Color', mc(i,:),'LineWidth',1);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ],[MED(:,i) MED(:,i)],'Color', medc(i,:),'LineWidth',1);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],'Color', mc(i,:),'LineWidth',1);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i))+x(i)-i],[MED(:,i) MED(:,i)],'Color', medc(i,:),'LineWidth',1);
        end
    end
end
