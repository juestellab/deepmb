...
class DelayNoSumGather(nn.Module):
  ...
  def __init__(self, transducer_pixel_distances, scanner_sampling_frequency, samples_offset):
    ...
    self.nb_pixels_z, self.nb_pixels_x, self.nb_elements = self.tof.transducer_pixel_distances.shape
  ...
  def forward(self, input_sinogram, sos):
    ...
    tof_floor = tof_floor.view(batch_size, self.nb_pixels_z * self.nb_pixels_x, self.nb_elements)
    tof_ceil = tof_ceil.view(batch_size, self.nb_pixels_z * self.nb_pixels_x, self.nb_elements)
    tof_floor_factor = tof_floor_factor.view(batch_size, self.nb_pixels_z * self.nb_pixels_x, self.nb_elements)
    tof_ceil_factor = tof_ceil_factor.view(batch_size, self.nb_pixels_z * self.nb_pixels_x, self.nb_elements)
    ...
    ima = ima.view((
      batch_size, self.nb_pixels_z, self.nb_pixels_x, self.nb_elements)).permute(0, 3, 1, 2).contiguous().float()
