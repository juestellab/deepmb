...
for speedOfSound = speed_of_sound_range
    ...
    if use_all_sos_instead_of_only_gt_sos_for_invivo_rec
        ...
        selmats_with_current_rec_sos = find(speed_of_sounds_invivo_gt==speedOfSound);
    ...
    for i_frame_with_current_rec_sos = selmats_with_current_rec_sos
        ...
        name_part_1 = [data_name_prefix names{i_frame_with_current_rec_sos} '_sosGt_' num2str(sos_gt) '_sosRec_' num2str(speedOfSound)];
        ...
        for wavelength = 1:size(data_preprocessed, 3)
            name = [name_part_1 func_name_pulse_suffix(wavelength)];
            writematrix([sos_gt; speedOfSound], fullfile(output_root_folder, split, subfolder_sos, [name '.csv']));
        end
        ...
        rec_img = reconstruct_model_based(model, data_preprocessed);
        for wavelength = 1:size(data_preprocessed,3)
            name = [name_part_1 func_name_pulse_suffix(wavelength)];
